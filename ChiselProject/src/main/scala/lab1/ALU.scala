...
class ALU(n: Int, m: Int, k: Int) extends Module {
  ...
  val sADD :: sMINUS :: sAND :: sOR :: sNOT :: sXOR :: Nil = Enum(6)
...
//  printf(p"arith_result = $arith_result\n")

  flag_zero := io.y === 0.U
//  flag_sign := io.y(31)
//  flag_overflow := (io.a(n - 1) & io.b(n - 1) & ~io.y(n - 1)) | (~io.a(n - 1) & ~io.b(n - 1) & io.y(n - 1))
...
object Lab1_1 extends App {
  chisel3.Driver.execute(args, () => new ALU(n = 6, m = 3, k = 3))
}
